    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Old invoice import') }}
            </h2>
        </div>
    </x-slot>
    <div class="container">
        <h1>Create Archived Invoice</h1>

        <form action="{{ route('archived-invoices.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="amount_incl"
                       class="form-control @error('amount_incl') is-invalid @enderror" value="{{ old('amount_incl') }}"
                       required>
                @error('amount_incl')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="amount_excl"
                       class="form-control @error('amount_excl') is-invalid @enderror" value="{{ old('amount_excl') }}"
                       required>
                @error('amount_excl')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="tax_amount"
                       class="form-control @error('tax_amount') is-invalid @enderror" value="{{ old('tax_amount') }}"
                       required>
                @error('tax_amount')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
